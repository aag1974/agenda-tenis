...
function toggleGearMenu() {
  ...
  const userHeader = state.user && el('div', { class: 'px-3 py-3 border-b border-slate-200 flex items-center gap-3 bg-slate-50' },
    ...
    el('div', { class: 'min-w-0 flex-1' },
      displayName && el('div', { class: 'text-sm font-bold text-slate-900 truncate' }, displayName),
      el('div', { class: `${displayName ? 'text-xs text-slate-500' : 'text-sm font-bold text-slate-900'} truncate` }, state.user.email || 'Conta'),
      planLine && el('button', {
  ...
  const athleteHeader = profile && el('div', { class: 'px-3 py-2.5 border-b border-slate-200' },
    ...
    el('button', {
      ...
      el('span', { class: 'flex items-center gap-1.5 min-w-0 text-sm text-slate-900 truncate' },
